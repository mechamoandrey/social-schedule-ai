...
function classifyType(title = '', arte = '') {
  ...
  if (
    /\bcampanha|outubro rosa|black friday|ação\b/.test(t) ||
    /campanha/.test(a)
  )
    return 'campanha';
  if (/^\s*dica\b|\bdicas\b|\bcomo\b|\bguia\b|checklist/i.test(title))
    return 'dica';
  if (/\bsobre a empresa\b|\binstitucional\b|quem somos/.test(t))
...
export function computeQuality({
  ...
  posts.forEach((p, idx) => {
    ...
    if (!(p.date && /^\d{4}-\d{2}-\d{2}$/.test(p.date))) {
      ...
      issues.push({
        postIndex: idx,
        type: 'date',
        message: `Data ${p.date} fora do mês ${month}.`,
      });
    }
    ...
    if (t.length < 5 || t.length > 90) {
      ...
      issues.push({
        ...
        type: 'length',
        field: 'title',
        message: `Título com ${t.length} caracteres (5–90).`,
      });
    }
    ...
    if (a.length === 0 || a.length > 200) {
      ...
      issues.push({
        ...
        type: 'length',
        field: 'arte',
        message: `Arte vazia ou >200 (${a.length}).`,
      });
    }
    ...
    if (!arteHasPrefix(a)) {
      ...
      issues.push({
        postIndex: idx,
        type: 'arte',
        message: "Arte deve começar com 'Carrossel:' ou 'Estático:'.",
      });
    }
    ...
    if (l.length === 0 || l.length > 500) {
      ...
      issues.push({
        ...
        type: 'length',
        field: 'legenda',
        message: `Legenda vazia ou >500 (${l.length}).`,
      });
    }
  ...
  for (let i = 1; i < posts.length; i++) {
    const prev = norm(posts[i - 1].cta).toLowerCase(),
      cur = norm(posts[i].cta).toLowerCase();
  ...
  posts.forEach((p, idx) => {
    ...
    if (tp === 'dica') {
      const ok = /temos\s+(este|esse)?\s*(material|item|produtos?)/i.test(
        p.arte || ''
      );
      if (!ok)
        issues.push({
          ...
          type: 'dica',
          message:
            "Post de dica sem variação de 'Temos este material' na ARTE.",
        });
    }

    if (tp === 'institucional' && site) {
      const inText =
        (p.legenda || '').includes(site) || (p.cta || '').includes(site);
      if (!inText)
        issues.push({
          postIndex: idx,
          type: 'institucional',
          message: 'Institucional não cita o site do cliente.',
        });
    }
  });
  ...
  if (titleUniqueRate < 0.85)
    varietyOk -= Math.round((0.85 - titleUniqueRate) * 40); // até -40
  ...
  // --- Holidays ---
  const suggested = Array.isArray(schedule?.suggested_holidays)
    ? schedule.suggested_holidays
    : [];
  ...
  const score = Math.round(
    0.3 * ruleOk + 0.3 * varietyOk + 0.25 * distOk + 0.15 * holOk
  );
  const grade =
    score >= 85
      ? 'Excelente'
