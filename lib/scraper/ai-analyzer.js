export async function analyzeWithAI(pagesData, options = {}) {
  ...
  const contentToAnalyze = allText.length > maxChars
    ? allText.slice(0, maxChars) + '\n\n[... conteúdo truncado ...]'
    : allText;

  const prompt = buildAnalysisPrompt(contentToAnalyze, pagesData[0]?.metadata);

  console.log(`[AI Analyzer] Sending ${contentToAnalyze.length} characters to OpenAI...`);
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'Você é um assistente especializado em analisar websites de empresas e extrair informações estruturadas.',
            },
            {
              role: 'user',
              content: prompt,
      ...
      lastError = error;
      console.error(`[AI Analyzer] Attempt ${attempt}/${maxAttempts} failed:`, error.message);
  ...
  throw new Error(`Falha ao analisar com IA após ${maxAttempts} tentativas: ${lastError.message}`);
}
