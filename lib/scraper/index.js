...
export async function scrapeWebsite(websiteUrl, options = {}) {
  ...
  try {
    ...
    if (menuLinks.length === 0) {
      console.log('[Scraper] ⚠ No menu links found, proceeding with homepage only');
    }

    console.log(`[Scraper] Step 4/5: Scraping ${menuLinks.length} submenu pages...`);

    for (let i = 0; i < menuLinks.length; i++) {
      const link = menuLinks[i];

      try {
        console.log(`[Scraper] (${i + 1}/${menuLinks.length}) Scraping: ${link.text}...`);
        ...
        console.log(`[Scraper] ✓ ${link.text} scraped (${pageData.text.length} chars)`);

        await wait(SCRAPING_CONFIG.delayBetweenPages);

      } catch (error) {
        console.error(`[Scraper] ✗ Failed to scrape ${link.href}:`, error.message);
      }
    }
    ...
    console.log(`  Products: ${analyzedData.products_or_services?.length || 0}`);
    console.log(`========================================\n`);
