...
export async function setupBrowser() {
  ...
  if (isDevelopment) {
    ...
    console.log('[Scraper] Launching browser in PRODUCTION mode (AWS Lambda)...');
...
export async function navigateToUrl(page, url, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`[Scraper] Navigating to ${url} (attempt ${attempt}/${maxRetries})...`);
      ...
      lastError = error;
      console.error(`[Scraper] Error navigating to ${url} (attempt ${attempt}):`, error.message);
