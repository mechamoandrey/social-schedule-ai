...
export default function EditOnePostModal({
  open,
  initialPost,
  onCancel,
  onInsert,
  ...
  function validate() {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(post.date || ''))
      return 'Data deve ser YYYY-MM-DD';
    if (!post.title || post.title.length < 5 || post.title.length > 90)
      return 'Título deve ter 5–90 caracteres';
    if (!post.arte || post.arte.length > 200)
      return 'Arte é obrigatória e ≤ 200 caracteres';
    if (!/^\s*(Carrossel:|Estático:)\s*/i.test(post.arte))
      return 'Arte deve começar com "Carrossel:" ou "Estático:"';
    if (!post.legenda || post.legenda.length > 500)
      return 'Legenda é obrigatória e ≤ 500 caracteres';
    if (post.cta && post.cta.length > 150)
      return 'CTA deve ter ≤ 150 caracteres';
    return '';
  }
