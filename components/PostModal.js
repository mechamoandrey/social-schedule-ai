    ...
    <div className='fixed inset-0 bg-black/40 grid place-items-center p-4 z-50'>
      <div className='bg-white w-full max-w-2xl rounded-xl shadow-lg overflow-hidden'>
        <div className='px-4 py-3 border-b flex items-center justify-between'>
          <h3 className='font-semibold'>Post • {form.title || 'Sem título'}</h3>
          <button
            onClick={onClose}
        ...
        <div className='p-4 space-y-3'>
          ...
          <div className='flex gap-2 text-sm mb-2'>
            <button
              className={`border rounded px-2 py-1 ${tab === 'dados' ? 'bg-neutral-100' : ''}`}
              onClick={() => setTab('dados')}
            >
              Dados
            </button>
            <button
              className={`border rounded px-2 py-1 ${tab === 'hist' ? 'bg-neutral-100' : ''}`}
              onClick={() => setTab('hist')}
            >
              Histórico
            </button>
          ...
          {tab === 'hist' ? (
            <div className='max-h-72 overflow-auto text-sm'>
              ...
              {!loadingHist && !history?.length && (
                <div className='text-neutral-500'>Sem eventos.</div>
              )}
              <ul className='space-y-2'>
                {history?.map(h => (
                  <li key={h.id} className='border rounded p-2'>
                    ...
                    {h.meta && (
                      <pre className='text-xs bg-neutral-50 p-2 rounded mt-1 overflow-auto'>
            ...
            <>
              ...
              <label className='text-sm'>
                ...
                />
                <div
                  className={`text-xs mt-1 ${overLegend ? 'text-red-600' : 'text-neutral-500'}`}
                >
              ...
              <label className='text-sm'>
                ...
                />
                <div
                  className={`text-xs mt-1 ${overCTA ? 'text-red-600' : 'text-neutral-500'}`}
                >
