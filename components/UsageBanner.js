export default function UsageBanner({ usage, onManage }) {
  ...
  const out = pct >= 100 && !usage.overage_enabled && !(usage.trial_end_at && new Date(usage.trial_end_at) > new Date());
  const color = out ? 'bg-rose-100 text-rose-800 border-rose-200' : low ? 'bg-amber-50 text-amber-800 border-amber-200' : 'bg-neutral-50 text-neutral-700 border-neutral-200';
    ...
    <div className={`border rounded-md px-3 py-2 ${color}`}>
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm">
          ...
          <span className="ml-1">{used}/{quota}</span>
          {usage.trial_end_at && (
            <span className="ml-2 text-xs">· Trial até {new Date(usage.trial_end_at).toLocaleDateString()}</span>
          )}
      ...
      {out && (
        <div className="mt-1 text-xs">Cota esgotada. Ative overage, aguarde renovação mensal ou faça upgrade.</div>
      )}
    </div>
  );
}
