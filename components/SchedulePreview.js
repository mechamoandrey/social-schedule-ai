export default function SchedulePreview({ schedule, onInsertOne, onEditOne }) {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="text-sm text-neutral-500">Total de posts: {posts.length}</div>
        ...
        {posts.map((p, i) => (
          <article key={i} className="border rounded-lg bg-white p-3">
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="text-[11px] text-neutral-500">Status: {p.status}</div>
              {onInsertOne && (
                <div className="flex gap-2">
                  <button 
                    onClick={() => onEditOne ? onEditOne(p, i) : onInsertOne(p, i)} 
