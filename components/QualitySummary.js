...
export default function QualitySummary({ schedule, clientWebsite, approvedHolidays, freqPerWeek }) {
  if (!schedule) return null;
  
  const q = computeQuality({ schedule, clientWebsite, approvedHolidays, freqPerWeek });
  ...
  return (
    <div className="border rounded-lg p-4 bg-white space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm">
          ...
          <span className={`ml-2 text-xs px-2 py-0.5 rounded ${
            q.score >= 85 ? 'bg-green-100 text-green-800' :
            q.score >= 70 ? 'bg-blue-100 text-blue-800' :
            q.score >= 50 ? 'bg-yellow-100 text-yellow-800' :
      ...
      <div className="grid md:grid-cols-3 gap-3 text-sm">
        <Card title="Formatos">
          Carrossel {q.stats.formatMix.carrossel} · Estático {q.stats.formatMix.estatico}
        </Card>
        <Card title="CTAs">
          Únicos {(q.stats.ctaUniqueRate * 100).toFixed(0)}% · Repetições consecutivas {q.stats.consecutiveCtaRepeats}
        </Card>
        <Card title="Títulos">
      ...
      <div className="grid md:grid-cols-3 gap-3 text-sm">
        <Card title="Posts por semana">
          W1:{q.stats.byWeek[0]} · W2:{q.stats.byWeek[1]} · W3:{q.stats.byWeek[2]} · W4:{q.stats.byWeek[3]} · W5:{q.stats.byWeek[4]}
        </Card>
        ...
        <Card title="Clusters">
          {q.stats.clusters.length ? 
            q.stats.clusters.map(c => `${c.start}–${c.end} (${c.count})`).join(', ') : 
            'Nenhum'
          }
        </Card>
      </div>
      ...
      <div className="text-sm">
        ...
        <div className="text-neutral-700">
          Sugeridos: {q.holidays.suggested.length} · Aprovados: {q.holidays.approved.length} · Usados em posts: {q.holidays.usedInPosts.length}
        </div>
      </div>
      ...
      <div className="text-sm">
        ...
        {q.issues.length === 0 ? (
          ...
          <ul className="list-disc ml-5 space-y-1">
            ...
            {q.issues.length > 12 && (
              <li className="text-neutral-500">+ {q.issues.length - 12} mais…</li>
            )}
          </ul>
        )}
      </div>
