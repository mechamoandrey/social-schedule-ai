  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const { data, error } = await sb.rpc('get_invitation_info', { p_token: token });
    ...
    <Layout>
      ...
      {info && (
        <div className="border rounded p-3 bg-white">
          <h2 className="font-semibold mb-2">Convite para {info.agency_name}</h2>
          <div className="text-sm text-neutral-600 mb-2">
            Você foi convidado para participar da agência <strong>{info.agency_name}</strong> como <strong>{info.role}</strong>.
          </div>
          <div className="text-xs text-neutral-500 mb-3">
            Convite enviado em: {new Date(info.created_at).toLocaleDateString()}<br />
          ...
          <div className="mt-3 space-y-3">
            {authUser ? (
              ...
              <div className="border rounded p-3 bg-neutral-50">
                <div className="text-sm font-medium mb-1">Entre para aceitar o convite</div>
                {!sent ? (
