...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!token || !postId || !action) {
      return res
        .status(400)
        .json({ error: 'token, postId e action são obrigatórios' });
    }
    ...
    if (!['approve', 'request_changes'].includes(action)) {
      return res
        .status(400)
        .json({ error: 'Ação inválida. Use approve ou request_changes' });
    }
    ...
    if (action === 'request_changes' && !comment?.trim()) {
      return res
        .status(400)
        .json({ error: 'Comentário é obrigatório para pedir ajustes' });
    }
    ...
    if (error) {
      ...
      if (error.message?.includes('Link inválido ou expirado')) {
        return res
          .status(403)
          .json({ error: 'Link de revisão inválido ou expirado' });
      }
      if (error.message?.includes('Post não pertence a este projeto')) {
        return res
          .status(409)
          .json({ error: 'Post não pertence a este projeto' });
      }
    ...
    if (process.env.NODE_ENV !== 'production') {
      console.log('[REVIEW_ACTION] Success:', {
        postId,
        action,
