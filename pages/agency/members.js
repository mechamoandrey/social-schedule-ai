...
export default function Members() {
  ...
  async function loadAgencyData(sb, aid) {
    try {
      ...
      const { data: ms, error: eM } = await sb.rpc('list_agency_members', {
        p_agency_id: aid,
      });
      ...
      const { data: iv, error: eI } = await sb.rpc('list_invitations', {
        p_agency_id: aid,
      });
          ...
          <div className='grid md:grid-cols-2 gap-4'>
              ...
              <form onSubmit={createInvite} className='space-y-2'>
                <label className='text-sm block'>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setFormInvite(f => ({ ...f, email: e.target.value }))
                    }
                  />
                </label>
                <label className='text-sm block'>
                  ...
                  <select
                    ...
                    onChange={e =>
                      setFormInvite(f => ({ ...f, role: e.target.value }))
                    }
                  >
                    <option value='social_media'>Social media</option>
                    <option value='client_viewer'>
                      Cliente (apenas aprovação)
                    </option>
                ...
                <label className='text-sm block'>
                  ...
                  <input
                    ...
                    onChange={e =>
                      setFormInvite(f => ({ ...f, days: e.target.value }))
                    }
                  />
                </label>
                <button className='border rounded px-3 py-2 text-sm hover:bg-neutral-50'>
            ...
            <section className='border rounded p-3 bg-white'>
              ...
              <ul className='space-y-2'>
                {(invites || []).map(iv => {
                  const status = iv.accepted_at
                    ? 'accepted'
                    : new Date() > new Date(iv.expires_at)
                      ? 'expired'
                      : 'pending';
                  return (
                    <li
