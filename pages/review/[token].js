...
export default function PublicReview() {
  ...
  async function approve(postId) {
    try {
      ...
      setPosts(prev =>
        prev.map(p =>
          p.id === postId
            ? { ...p, status: 'Aprovado', revision_comment: null }
            : p
        )
      );
      alert('Aprovado!');
  ...
  async function requestChanges(postId) {
    try {
      ...
      setPosts(prev =>
        prev.map(p =>
          p.id === postId
            ? { ...p, status: 'Ajustar', revision_comment: comment }
            : p
        )
      );
      setComments(prev => {
  ...
  if (!hdr)
    return <div className='p-6 text-sm'>Link inválido ou expirado.</div>;

  return (
    <div className='max-w-3xl mx-auto p-4'>
      <h1 className='text-xl font-semibold'>
        {hdr.client_name} — {hdr.project_name}{' '}
        <span className='text-sm text-neutral-500'>
          ({posts?.length || 0} posts)
        </span>
      </h1>
      <div className='text-sm text-neutral-600 mb-3'>
        Mês: {hdr.month} · Expira em:{' '}
        {new Date(hdr.expires_at).toLocaleString()}
        {hdr.expired ? ' · (Expirado)' : ''}
      </div>

      <div className='border rounded p-3 mb-4 bg-white'>
        <div className='text-sm font-medium mb-2'>
          Identifique-se (opcional)
        </div>
        <input
          value={actor}
        ...
        {posts.map(p => (
          <li key={p.id} className='border rounded p-3 bg-white'>
            ...
            <div className='mt-3 flex flex-col gap-2'>
              ...
              <label className='text-xs' htmlFor={`comment-${p.id}`}>
                Comentário
              </label>
              <textarea
                id={`comment-${p.id}`}
